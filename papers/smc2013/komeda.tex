...
are effortlessly expandable with various sensors and controllers.  Moreover
they are affordable, which creates possibility to build networks of cooperating
devices.

In this paper we describe the design of Komeda -- implementation platform for
interactive algorithmic music adapted for embedded systems.  The framework
consists of music description language, intermediate binary representation and
portable virtual machine with user defined add-ons (aka modules).
\end{abstract}
...
installation.  Mobile devices (phones, tablets etc.) are not well suited for
